...
def container_image
  output = `podman image exists #{CACHED_IMAGE} 2>&1`
...
namespace :test do
  ...
  desc "Run containerized integration tests (requires Podman)"
  task :containers do
    ...
    puts image == CACHED_IMAGE ? "(cached)" : "(no cache — run `rake test:containers:build` to speed this up)"
    ...
    abort "\n#{failures.length} container test(s) failed" unless failures.empty?
    puts "\nAll #{results.length} container tests passed"
  end
end
