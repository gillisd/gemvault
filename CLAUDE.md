...
- `lib/rubygems_plugin.rb` and every file it reaches use `require_relative`, plus explicit `require "rubygems/..."` for the RubyGems files they patch — Bundler loads a path gem's plugin by absolute path with nothing activated and lib/ off the load path; the gemvault-core requires defer to a copy already loaded (first copy wins, as in the shim), so a superseded install's plugin stub cannot clash with the shim's pinned gemvault (issue #32)
