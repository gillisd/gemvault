...
class IntegrationTest < Minitest::Test
  ...
  INTEGRATION_TMP = (Pathname(__dir__).parent / "tmp" / "integration").freeze
  PLUGIN_PATH = Pathname(__dir__).parent.freeze

  def setup
    @project_dir = INTEGRATION_TMP / "test_#{name}_#{$PROCESS_ID}_#{Time.now.to_i}"
    @project_dir.mkpath
  ...
  def test_plugins_rb_makes_plugin_root_deps_activatable
    ...
    script_path.write(<<~RUBY)
      ...
      # Expected it only exists in the fake Plugin.root      end
      ...
      # Run the plugins.rb preamble the code before require_relative.      # In the broken version: no-op (just comments).
