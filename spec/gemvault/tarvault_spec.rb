...
RSpec.describe Gemvault::Tarvault do
  ...
  def gem_entry(name:, version:) = Gemvault::GemEntry.new(name: name, version: version)

  let(:foo_entry) { gem_entry(name: "foo", version: "1.0.0") }
  ...
  describe "lifecycle", :aggregate_failures do
    ...
    it "reopens and preserves data" do
      create { |v| v.add(build_gem(name: "foo", version: "1.0.0")) }
      described_class.open(vault_path) { |v| expect(v.size).to eq(1) }
    end
  end

  describe "#add" do
    let(:gem_file) { build_gem(name: "foo", version: "1.0.0") }
    ...
    it "raises Vault::DuplicateGemError on the same name/version/platform" do
      gem = build_gem(name: "foo", version: "1.0.0")
      create do |v|
        v.add(gem)
    ...
    it "stores a platform-specific gem under its platform filename" do
      create { |v| v.add(build_gem(name: "native", version: "1.0.0", platform: "x86_64-linux")) }
  ...
  describe "#remove" do
    ...
    it "removes all versions by name and returns the count" do
      create do |v|
        v.add(build_gem(name: "foo", version: "1.0.0"))
        v.add(build_gem(name: "foo", version: "2.0.0"))
  ...
  describe "#gem_data", :aggregate_failures do
    it "returns the exact stored bytes" do
      ...
      create { |v| v.add(gem) }
      described_class.open(vault_path) { |v| expect(v.gem_data(foo_entry)).to eq(original) }
    end

    it "raises Vault::NotFoundError for a missing gem" do
      create { |v| expect { v.gem_data(gem_entry(name: "nope", version: "1.0.0")) }.to raise_error(Gemvault::Vault::NotFoundError) }
    end
    ...
    it "raises Vault::Error when the stored blob fails its checksum" do
      ...
      described_class.open(vault_path) do |v|
        expect { v.gem_data(foo_entry) }.to raise_error(Gemvault::Vault::Error, /[Ii]ntegrity/)
      end
    end
  end
  ...
  describe "#gem_entries / #specs" do
    ...
    it "loads gemspecs including dependencies and platform" do
      create { |v| v.add(build_gem(name: "dep", version: "1.0.0", dependencies: [["rake", ">= 13.0"]])) }
      described_class.open(vault_path) do |v|
  ...
  describe "GemExtraction" do
    it "yields a real .gem file path from #with_gem_file and unlinks it after" do
      create { |v| v.add(build_gem(name: "foo", version: "1.0.0")) }
      captured = nil
      described_class.open(vault_path) { |v| v.with_gem_file(foo_entry) { |p| captured = p } }
